...
typedef enum eAsmBinarytokentype {
	...
	eAsmBinarytokentype_Position,    // Indexed
	eAsmBinarytokentype_Expr,    // Indexed
} eAsmBinarytokentype;
...
tAsmBinarytoken* mtAsmBinarytoken_Get(FILE* src){ // Constructor
	...
	switch(fpeekc(src)){
		...
		case '#':
#ifdef qvGTrace
			//printf("ASM:[T] mtAsmBinarytoken_Get: Argument \n");
#endif
			fgetc(src);
			return mtAsmBinarytoken_Clone(
				&(tAsmBinarytoken){
					.type = eAsmBinarytokentype_Expr,
					.disp = fgetc(src)-'0'
				}
			);
			break;
...
void mtAsmBinarytoken_Emit(tAsmBinarytoken* self,FILE* dst){
	...
	switch(self->type){
		...
		default:
			printf("ASM:[E] mtAsmBinarytoken_Emit: Unrecognized token type %i\n",self->size);
			break;
	};
	switch(self->size){
		case eAsmBinarytokensize_Lobyte:
...
#ifdef qvGTrace
					printf("ASM:[F] AsmReadinstructiondefinition: Fetched expansion token %s \n",mtAsmBinarytoken_ToString(bintoken));
#endif
					mtList_Append(instruction->expansion,bintoken);
				};
...
bool AsmInstructionfinderclojure(
	...
	// Found it.
#ifdef qvGTrace
	printf("ASM:[T] AsmInstructionfinderclojure:   Found instrdef \n");
#endif
	return true;
};
...
void AsmSecondpassline(FILE* src,FILE* dst){
	//
#ifdef qvGTrace
	...
	tAsmToken* tok = mtAsmToken_Get(src);
	if(!tok)return;
	if(tok->type==eAsmTokentype_Newline)return;
	...
	if(tok->type!=eAsmTokentype_Identifier){
		printf("ASM:[F] AsmSecondpassline: Unrecognized token while parsing instruction opcode \n");
	...
	// Clear out bound arguments
	for(int i=0;i<10;i++){
		AsmBoundparameters[i]=0;
	};
	// Find expansion
	...
	if(!instrdef){
		printf("ASM:[E] AsmSecondpassline: Undefined instruction < ");
		printf("%s | ", opcode);
...
void AsmFirstpassline(FILE* src){
	//
#ifdef qvGTrace
...
#endif
	while(fpeekc(srcfile)!=EOF)AsmFirstpassline(srcfile);
	// Interpass seek
...
#endif
	while(fpeekc(srcfile)!=EOF)AsmSecondpassline(srcfile,dstfile);
	// Close it all
	fclose(srcfile);
	fclose(dstfile);
	return 0;
